...
- (void) touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (1 == tc)
    {
        CGPoint touchPoint  = [touch locationInView:overlayViewController.view];
        if ((touchPoint.x >= 0.0f) && (touchPoint.x < 60.0f) && (touchPoint.y >= 0.0f) && (touchPoint.y < 60.0f))
        {
            [overlayViewController closeAction:self];
            [self arClose];
            return;
        }
