...
#import "QCARutils.h"
#import "AUIExtensionsKit.h"

@interface OverlayViewController ()
{
    BOOL    afterFirstInstall;
}
...
- (void) loadView
{
    ...
    optionsOverlayView = (OverlayView*)[OverlayView viewWithNibName:@"OverlayView" owner:self];
    optionsOverlayView.frame            = screenBounds;
    optionsOverlayView.parentController = self;
...
{
    optionsOverlayView.btnClose.alpha           = 0.0f;
    optionsOverlayView.lblInitializing.alpha    = 1.0f;

    [optionsOverlayView.aiPending startAnimating];
}

- (void)viewDidAppear:(BOOL)animated
{
    [UIView animateWithDuration:0.5f
                          delay:(afterFirstInstall ? 0.0f : 2.0f)
                        options:UIViewAnimationCurveEaseInOut
                     animations:^
    {
        optionsOverlayView.btnClose.alpha   = 0.5f;
    }
                     completion:^(BOOL finished)
    {
        afterFirstInstall = YES;

        [optionsOverlayView.aiPending stopAnimating];
        
        [UIView animateWithDuration:0.3f
                         animations:^
         {
             optionsOverlayView.lblInitializing.alpha    = 0.0f;
         }];
    }];
}
