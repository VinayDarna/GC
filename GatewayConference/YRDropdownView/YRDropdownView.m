...
{
    [self hideUsingAnimation:[NSNumber numberWithBool:animated]];
}
...
- (void)layoutSubviews {    
    ...
    if (self.detailText) {
        ...
        CGColorRef color2 = [detailLabel.textColor CGColor];
        const CGFloat *components2 = CGColorGetComponents(color2);
        float total2 = 0.0;
        for (int i = 0; i < (int)CGColorGetNumberOfComponents(color2)-1; i++){
            total2 += components2[i];
        }
        if ((total2/((int)CGColorGetNumberOfComponents(color2)-1)) > 0.5){
    ...
    if (self.accessoryImage) {
        float   imagePadding = IMAGE_PADDING;
        if (self.accessoryImage.size.width > 40)
        {
            imagePadding    = 80;
        }
        ...
        [titleLabel sizeToFitFixedWidth:self.bounds.size.width - imagePadding - (HORIZONTAL_PADDING * 2)];
        titleLabel.frame = CGRectMake(titleLabel.frame.origin.x + imagePadding, 
                                      titleLabel.frame.origin.y, 
                                      titleLabel.frame.size.width, 
                                      titleLabel.frame.size.height);
        
        if (self.detailText) {
            [detailLabel sizeToFitFixedWidth:self.bounds.size.width - imagePadding - (HORIZONTAL_PADDING * 2)];
            detailLabel.frame = CGRectMake(detailLabel.frame.origin.x + imagePadding, 
    ...
    if (self.detailText) {
        ...
        dropdownHeight = MAX(dropdownHeight, CGRectGetMaxY(accessoryImageView.frame));
