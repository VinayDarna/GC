...
{
    if(definition && ![_dataDefinitions valueForKey:definition.dataStructureName])
...
- (void)asynchronousFetchObjectsWithOptions:(SCDataFetchOptions *)options success:(SCDataStoreFetchSuccess_Block)success_block failure:(SCDataStoreFailure_Block)failure_block
{
    // Must be implemented by subclasses that support SCDataStoreModeAsynchronous
    if(failure_block)
        failure_block(nil);
