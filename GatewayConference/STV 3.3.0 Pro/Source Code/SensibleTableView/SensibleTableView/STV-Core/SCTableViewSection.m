...
{
#ifdef SC_STVTRIAL
    if([self.ownerTableViewModel indexForSection:self]==self.ownerTableViewModel.sectionCount-1)
    {
        return [SCLicenseManager licenseViewForWidth:self.ownerTableViewModel.tableView.frame.size.width];
    }
#endif
...
- (void)setExpanded:(BOOL)expanded
{
    ...
    if(self.ownerTableViewModel.live)
    {
        ...
        if(expanded)
            ...
            [self.ownerTableViewModel.tableView deleteRowsAtIndexPaths:indexPaths withRowAnimation:UITableViewRowAnimationAutomatic];
...
- (id)initWithHeaderTitle:(NSString *)sectionHeaderTitle boundObject:(NSObject *)object
{
...
#ifdef SC_STVLITE
    if([SCLicenseManager isUnlicensedObject:self.boundObject])
    {
        [self addCell:[SCTableViewCell cellWithText:[SCLicenseManager nameForUnlicensedObject:self.boundObject]]];
        return;
    }
#endif
                                        ...
                                        inEditingMode:(BOOL)editing
{
    ...
    if(propertyType == SCPropertyTypeAutoDetect)
    {
        ...
        if(propertyDataType == SCDataTypeNSString)
        {
            if(readOnlyProperty)
    ...
    switch (propertyType)
    {
        ...
        case SCPropertyTypeObjectSelection:
            ...
            {
                cell = [propertyDefinition.uiElementClass cellWithText:propertyTitle boundObject:boundObj selectedObjectPropertyName:propertyName selectionItemsStore:nil];
            }
            else
            {
        ...
        case SCPropertyTypeObject:
        {
            ...
            else
            {
                ...
                {
                    cell = [propertyDefinition.uiElementClass cellWithBoundObject:object boundObjectDefinition:objDef];
                }
                else
                {
        ...
        case SCPropertyTypeArrayOfObjects:
        {
            ...
            {
                cell = [propertyDefinition.uiElementClass cellWithDataStore:objectsStore];
            }
            else
            {
...
@synthesize cellCountLimitForAutoHeight;
...
- (id)init
{
	if( (self=[super init]) )
	{
        ...
        cellIdentifier = (__bridge_transfer NSString *)CFUUIDCreateString(kCFAllocatorDefault, uuid);
        CFRelease(uuid);
        ...
        cellCountLimitForAutoHeight = 100;
        ...
        if([self.dataStore validateInsertForObject:tempItem])
        {
            switch (self.dataStore.storeMode)
            {
                ...
                case SCStoreModeAsynchronous:
                    [self.dataStore asynchronousInsertObject:tempItem success:nil failure:nil];
                    break;
            }
        }
        else 
...
- (void)setCellCountLimitForAutoHeight:(NSUInteger)limit
{
    cellCountLimitForAutoHeight = limit;
...
- (CGFloat)heightForCellAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(_cachedCellHeight)
        return _cachedCellHeight;
    
    cellHeight = [super heightForCellAtIndexPath:indexPath];
    if(self.cellCount > self.cellCountLimitForAutoHeight)
...
- (void)didFetchItems:(NSArray *)fetchedItems sender:(id)sender
{
    ...
    if(sender!=self || self.dataStore.storeMode==SCStoreModeAsynchronous)
    {
        ...
        {
            // reload section
            [self.ownerTableViewModel.tableView reloadSections:[NSIndexSet indexSetWithIndex:sectionIndex] withRowAnimation:UITableViewRowAnimationNone];
        }
        else 
        {
            NSMutableArray *indexPaths = [NSMutableArray array];
...
- (BOOL)shouldDeleteItem:(NSObject *)item atIndexPath:(NSIndexPath *)indexPath
{
	...
	{
		shouldDelete = [self.ownerTableViewModel.delegate
...
- (SCTableViewCell *)unconfiguredCellAtIndex:(NSUInteger)index;
{
	...
	if([self.ownerTableViewModel.dataSource conformsToProtocol:@protocol(SCTableViewModelDataSource)]
	   && [self.ownerTableViewModel.dataSource respondsToSelector:@selector(tableViewModel:reuseIdentifierForRowAtIndexPath:)])
	{
		cellId = [self.ownerTableViewModel.dataSource tableViewModel:self.ownerTableViewModel reuseIdentifierForRowAtIndexPath:indexPath];
	}
	if(!cellId)
	{
    ...
    if(cell == nil) 
	{
		...
		if([self.ownerTableViewModel.dataSource conformsToProtocol:@protocol(SCTableViewModelDataSource)]
		   && [self.ownerTableViewModel.dataSource respondsToSelector:@selector(tableViewModel:cellForRowAtIndexPath:)])
		{
			cell = [self.ownerTableViewModel.dataSource tableViewModel:self.ownerTableViewModel cellForRowAtIndexPath:indexPath];
		}
...
{
	SCTableViewCell *cell = [[SCTableViewCell alloc] initWithStyle:SC_DefaultCellStyle reuseIdentifier:self.cellIdentifier];
	cell.ownerTableViewModel = self.ownerTableViewModel;
    cell.ownerSection = self;
...
- (void)commitAndProcessChangesForDetailModel:(SCTableViewModel *)detailModel
{
    if(self.selectedCellIndexPath)
    {
        ...
        if(!detailModel.needsCommit || ![self shouldUpdateItem:item atIndexPath:self.selectedCellIndexPath itemModel:detailModel])
        {
            return;
        }
    }
    ...
    if(self.selectedCellIndexPath)
	{
        ...
        switch (self.dataStore.storeMode)
        {
            ...
            case SCStoreModeAsynchronous:
                [self.dataStore asynchronousUpdateObject:item success:nil failure:nil];
                break;
        }
        
        if(self.sectionActions.didUpdateItem)
        {
            self.sectionActions.didUpdateItem(self, item, self.selectedCellIndexPath);
        }
        else
            if(self.ownerTableViewModel.sectionActions.didUpdateItem)
            {
                self.ownerTableViewModel.sectionActions.didUpdateItem(self, item, self.selectedCellIndexPath);
            }
            else
                if([self.ownerTableViewModel.delegate conformsToProtocol:@protocol(SCTableViewModelDelegate)]
                   && [self.ownerTableViewModel.delegate
...
- (UIViewController *)getDetailViewControllerForCell:(SCTableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath withItem:(NSObject *)item
{
    ...
    if(self.ownerTableViewModel.detailViewController && selectedCellIndexPath)
    {
    ...
    {
        self.sectionActions.detailModelConfigured(self, detailModel, indexPath);
    }
    else 
        if(self.ownerTableViewModel.sectionActions.detailModelConfigured)
        {
            self.ownerTableViewModel.sectionActions.detailModelConfigured(self, detailModel, indexPath);
        }
    else 
	if([self.ownerTableViewModel.delegate conformsToProtocol:@protocol(SCTableViewModelDelegate)]
       && [self.ownerTableViewModel.delegate 
...
- (void)dispatchEventSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	UIViewController *detailViewController = [self getDetailViewControllerForCell:cell forRowAtIndexPath:indexPath withItem:item];
...
- (void)dispatchEventAddNewItem
{
	...
	UIViewController *detailViewController = [self getDetailViewControllerForCell:nil forRowAtIndexPath:indexPath withItem:tempItem];
				...
				toIndexPath:(NSIndexPath *)toIndexPath 
{
	...
	{
		[self.dataStore changeOrderForObject:item toOrder:toIndexPath.row];
	...
	else
	{
        switch (self.dataStore.storeMode)
        {
            ...
            case SCStoreModeAsynchronous:
                [self.dataStore asynchronousDeleteObject:item success:nil failure:nil];
                break;
        }
...
- (NSObject *)createNewItem
{
	...
	{
		newItem = [self.ownerTableViewModel.dataSource tableViewModel:self.ownerTableViewModel
...
- (void)addCellForNewItem:(NSObject *)newItem
{
	...
	if(!self.autoSelectNewItemCell && !self.ownerTableViewModel.detailViewController)
	...
	{
        [self dispatchEventSelectRowAtIndexPath:newRowIndexPath];
	}
...
- (void)handleDetailViewControllerWillDismiss:(UIViewController *)detailViewController cancelButtonTapped:(BOOL)cancelTapped doneButtonTapped:(BOOL)doneTapped
{
    ...
    if(![self.ownerTableViewModel.viewController isKindOfClass:[UITableViewController class]])
    {
        ...
        if(selectedCellIndexPath)
            [self.ownerTableViewModel.tableView deselectRowAtIndexPath:selectedCellIndexPath animated:YES];
    }
}
...
- (void)handleDetailViewControllerDidLoseFocus:(UIViewController *)detailViewController cancelButtonTapped:(BOOL)cancelTapped doneButtonTapped:(BOOL)doneTapped
{
    if(self.selectedCellIndexPath && !self.ownerTableViewModel.detailViewController)
    {
...
{
    SCMemoryStore *arrayStore = [SCMemoryStore storeWithObjectsArray:sectionItems defaultDefiniton:definition];
...
{
    SCMemoryStore *stringsArrayStore = [SCMemoryStore storeWithObjectsArray:sectionItems defaultDefiniton:[SCStringDefinition definition]];
				...
				toIndexPath:(NSIndexPath *)toIndexPath
{
    ...
    NSNumber *oldItemIndex = [NSNumber numberWithInteger:fromIndexPath.row];
    NSNumber *newItemIndex = [NSNumber numberWithInteger:toIndexPath.row];
    if([self.selectedItemsIndexes containsObject:oldItemIndex] && [self.selectedItemsIndexes containsObject:newItemIndex])
    {
        // no need to update anything
    }
    else
        if([self.selectedItemsIndexes containsObject:oldItemIndex])
        {
            [self.selectedItemsIndexes removeObject:oldItemIndex];
            [self.selectedItemsIndexes addObject:newItemIndex];
        }
        else
            if([self.selectedItemsIndexes containsObject:newItemIndex])
            {
                [self.selectedItemsIndexes removeObject:newItemIndex];
                [self.selectedItemsIndexes addObject:oldItemIndex];
            }
...
- (id)initWithHeaderTitle:(NSString *)sectionHeaderTitle
              boundObject:(NSObject *)object selectedObjectPropertyName:(NSString *)propertyName
                    items:(NSArray *)sectionItems itemsDefintion:(SCDataDefinition *)definition
{
	SCMemoryStore *store = [SCMemoryStore storeWithObjectsArray:[NSMutableArray arrayWithArray:sectionItems] defaultDefiniton:definition];
